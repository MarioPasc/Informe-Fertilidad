...
Se intentará explicar el evento de la primera blastulación con las variables comunes a los pacientes. Por ello, de todas las variables con un p-valor compartido con  `BLASTUL_1ESTIM` menor que 0.05, solo nos quedaremos con: `EDAD`, `AFC` y `FACTOR_FEM_1`. 
...
```{r}
variables_significativas_2estim <- resultados %>%
  dplyr::filter(grepl("BLASTUL_2ESTIM", VariableX) | grepl("BLASTUL_2ESTIM", VariableY) &
                Valor < 0.05) %>%
  dplyr::select(VariableX, VariableY, Valor, Test) %>%
  dplyr::arrange(VariableY) %>%
