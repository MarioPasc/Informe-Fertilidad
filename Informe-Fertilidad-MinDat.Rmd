...
## Column 1 {data-width=75%}
-----------------------------------------------------------------------

**Abstract**

En el panorama actual de la salud reproductiva, las clínicas de fertilidad desempeñan un papel fundamental al ofrecer esperanza y soluciones avanzadas a individuos y parejas que enfrentan desafíos en su camino hacia la paternidad. Estas clínicas no solo son pilares de innovación tecnológica y médica en el tratamiento de la infertilidad, sino también centros de apoyo compasivo y orientación para aquellos en busca de concebir. La implementación de tratamientos **especializados, económicos y accesibles**, como los antagonistas de la GnRH y los gestágenos, refleja el compromiso de estas instituciones con la adaptación a las necesidades y circunstancias únicas de cada paciente, lo que subraya su importancia intrínseca en la mejora de la salud reproductiva y en el fortalecimiento de las familias.

**Contexto de desarrollo del Estudio**

Los antagonistas de la GnRH y los gestágenos son tratamientos de fertilidad que actúan sobre el sistema endocrino para optimizar la estimulación ovárica: los antagonistas de la GnRH inhiben de manera directa la secreción de gonadotropinas para prevenir la ovulación prematura, mientras que los gestágenos, al ser parte de los protocolos de progestina, pueden modificar el entorno hormonal favoreciendo un desarrollo folicular adecuado sin la supresión abrupta de la LH. La diferencia entre estos grupos radica principalmente en sus mecanismos de acción y efectos sobre el ciclo menstrual, lo que puede conducir a variaciones en la efectividad, la experiencia del paciente y los costos asociados. 

Este estudio se centra en evaluar y comparar la eficacia de dos tratamientos de fertilidad: los basados en **antagonistas de la GnRH (ANTAGONISTAS)** y los que utilizan **gestágenos (GESTAGENOS)**, con el fin de entender mejor cómo estos tratamientos influyen en los resultados de las estimulaciones ováricas. A través del análisis de variables comunes registradas en dos rondas de estimulación por paciente y su correlación con los eventos de blastulación, se pretende identificar los factores explicativos clave para el éxito del tratamiento. Al abordar específicamente la dinámica de la blastulación —la formación de una blástula que es un indicador crítico del éxito reproductivo—, el estudio aspira a desentrañar las complejidades subyacentes que distinguen la efectividad entre los dos grupos de tratamiento.

**Consecuencias del Estudio**

Desde una perspectiva económica, entender las diferencias en la efectividad entre los tratamientos con antagonistas de la GnRH y gestágenos puede facilitar una asignación más eficiente de recursos y reducir la carga financiera para las clínicas y las familias. En términos de salud mental, clarificar el impacto de estos tratamientos tiene el potencial de aliviar significativamente la tensión emocional que experimentan las parejas durante los procesos de fertilidad, ofreciendo esperanza y dirección más claras. Para las clínicas, la capacidad para ajustar y personalizar los tratamientos basados en evidencia robusta no solo mejora las tasas de éxito sino que también refuerza su reputación como líderes en cuidado reproductivo. Finalmente, este estudio contribuye a forjar un futuro más brillante para aquellos en la búsqueda de la paternidad, enfatizando el papel crítico de la investigación cuidadosa y la medicina basada en evidencia en la mejora continua de los tratamientos de fertilidad.

## Column 2 {data-width=23%}
-----------------------------------------------------------------------
...
<div style="text-align: center;">
    <img src="foto.jpg" alt="Texto alternativo" width="200"/>
</div>
')
```

Mario Pascual González es estudiante de Ingeniería de la Salud con mención en Bioinformática en la Universidad de Málaga. 

Ha participado en diversos proyectos enfocados en la Visión por Computador y el Machine Learning, tanto como parte de su plan de estudios como en iniciativas independientes. ([LinkedIn](https://www.linkedin.com/in/mario-pascual-gonzalez/)).

Sus principales áreas de interés incluyen el aprendizaje profundo, la aplicación de la visión por computador en el ámbito biomédico, la representación del conocimiento y razonamiento, sistemas cognitivos y el procesamiento del lenguaje natural.
...
# =======================================
# Obtención de estadísticas: Categóricas 
#========================================
...
```{r echo=TRUE, eval = FALSE}
# =======================================
#   Cálculo de Estadísticas: Numéricas
# =======================================
...
```{r echo=TRUE, eval = FALSE}
# =======================================
#   Gráfico Interactivo Shiny y Plotly
# =======================================
...
### Pseudocódigo decisión test estadístico
...
# Crear la gráfica de barras (Costó lo suyo)
ggplot(df_agregado, aes(fill = Significancia, y = N, x = Test)) +
  geom_bar(position = "stack", stat = "identity") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = "Tipo de Test",
  ...
  scale_fill_manual(values = c("p < 0.01" = "red", "p < 0.05" = "blue", "p >= 0.05" = "grey"))
```
Las únicas funciones a destacar son las de las gráficas. GGPLOT no es sencillo de usar, aunque se parezca mucho a mayplotlib.pyplot (con la que soy mucho más familiar). 
...
2. Partiendo de estas variables, que guardan una significancia estadística con blastulación, se construirá el modelo explicativo de regresión logística. 

Como se puede observar en la tabla superior, las variables que guardan una significacia estadística son:

- `BLASTUL_1ESTIM`: `EDAD`, `AFC`, `FACTOR_FEM_1`, `Nº_OVO_1ESTIM`, `Nº_OVO_1ESTIM`, `MII_1ESTIM`, `MII_1ESTIM`, `FERTILIZ_1ESTIM`, `FERTILIZ_2ESTIM`, `FET_1ESTIM`, `FET_2ESTIM`, `BT_AA_1ESTIM`, `CONG_1ESTIM`.
- `BLASTUL_2ESTIM`: `N_OVO_2ESTIM`, `MII_2ESTIM`, `FERTILIZ_2ESTIM`, `BT_AA_2ESTIM`, `CONG_2ESTIM`, `FET_2ESTIM`, `GRUPO`

### Modelo Explicativo - Blastulación 1ESTIM

**Objetivo Específico**


Se pretende explicar el evento de la primera blastulación con las variables comunes a los pacientes. Por ello, de todas las variables con un p-valor compartido con  `BLASTUL_1ESTIM` menor que 0.05, solo nos quedaremos con: `EDAD`, `AFC` y `FACTOR_FEM_1`. 


**Modelo - Interpretación**


En la columna central, en la pestaña "Modelo BLASTUL Primera Estimulación" se pueden encontrar los resultados de ajustar este modelo. Como se puede observar, en este modelo con todas las variables, la única que parece indicar tener una asociación con la blastulación en primera estimulación es `FACTOR_FEM_1ENDO`. La estimación de 1.6402 unidades indica que la presencia de este factor femenino, *endometriosis*, está asociada con la probabilidad positiva de que se produzca la blastulación. 


Adicionalmente, la varianza de los residuos del ajuste con las variables es 90.065, valor menor que la varianza con ninguna, `Null deviance`, que tiene un valor de 102.723. Esto es un indicador de que el modelo explica mejor el evento de blastulación con las variables añadidas. 


**Modelo - Ajuste**

Para explorar la posbilidad de que el modelo pueda explicar mejor la blastulación en ausencia de alguna de las variables se va a proceder aplicando un algoritmo "stepwise" hacia atrás ("backwards"). Se ha decidido aplicarlo hacia atrás ya que hemos probado que el modelo explica mejor el suceso cuando todas las variables están presentes, así que será más sabio partir de esta base e ir eliminando variables del modelo para comprobar si se influye una mejora en este.


Comparando los valores de la columna central y la de la derecha de la UI, el algoritmo stepwise converge a un modelo más simple, el cual consigue explicar el evento de blastulación con el uso de las variables `EDAD` y `FACTOR_FEM_1ENDO`, prescindiendo de la variable `AFC`. Adicionalmente, la significancia estadística de la variable `FACTOR_FEM_1ENDO` parece ser mayor, ya que el p-valor asociado a esta ha disminuído ligeramente a 0.0305, además, el incremento en el coeficiente estimado para esta variable subraya una asociación positiva más fuerte con el éxito de la blastulación


**Conclusión**


El modelo refinado mediante el algoritmo "stepwise" hacia atrás ofrece una visión concentrada sobre los predictores clave de la blastulación, destacando la importancia de la edad y factores femeninos específicos, como se ejemplifica en `FACTOR_FEM_1ENDO`. Este enfoque debe complementarse con una evaluación continua y un análisis detallado de todas las variables disponibles para asegurar una comprensión integral de los determinantes de la blastulación.


### Modelo Explicativo - Blastulación 2ESTIM

**Objetivo Específico**


Se pretende explicar el evento de la segunda blastulación con las variables comunes a los pacientes y las variables de primera estimulación. 


**Modelo - Interpretación**

Debido a que la única variable común o de primera estimulación que guarda una significacia estadística con `BLASTUL_2ESTIM` es `GRUPO`, el modelo solo puede construirse con esta variable. Aunque se procederá con este acercamiento para comprobar si existe alguna posibilidad de que el modelo converja en una solución. 

Observando los valores de la columna central, en concreto del apartado "Modelo BLASTUL Segunda Estimulación", podemos observar que la estimación es de 18.0906, siendo un coeficiente muy grande, pero dado el enorme error estándar (1496.3959), la interpretación se vuelve poco clara. Este coeficiente sugeriría que, en comparación con el grupo de referencia, la pertenencia al grupo tratado con gestágenos está asociada con un aumento en el log-odds de la blastulación en la segunda estimulación. Sin embargo, la interpretación directa de este coeficiente es imposible debido al p valor de 0.9904. Este p-valor extremadamente alto indica que no hay evidencia estadística para afirmar que la variable GRUPOGESTAGENOS tiene un efecto significativo sobre la probabilidad de blastulación en la segunda estimulación. La significancia estadística de esta variable es prácticamente nula en este modelo. 


**Modelo - Ajuste**


Para explorar la posibilidad de encontrar una solución, se va a probar a partir del modelo con la variable `GRUPO` y aplicar un algoritmo stepwise hacia adelante ("forward"). Esto se hará ya que existe una posibilidad de que el modelo explicativo converja en una solución al añadir más variables, aunque estas aumenten la complejidad e interpretabilidad de este. 


Sin embargo, si observamos los valores de salida del algoritmo en la columna de la derecha de la UI en el apartado "Ajuste Modelo Blastulación Segunda Estimulación", se puede concluir que el algoritmo no ha conseguido encontrar un mejor modelo que el propuesto en el anterior apartado, aunque este no sea válido. 


**Conclusión**


El modelo sugiere que no hay evidencia suficiente para afirmar que el grupo tratado con gestágenos tiene un impacto significativo en la probabilidad de blastulación en la segunda estimulación. El enorme error estándar asociado con el coeficiente de `GRUPOGESTAGENOS` indica una gran incertidumbre en la estimación de este efecto, lo que podría ser resultado de la variabilidad en los datos, un tamaño de muestra pequeño para este grupo específico, o la presencia de valores atípicos.


Es importante considerar estos resultados en el contexto de la investigación general sobre la fertilidad y los tratamientos de estimulación ovárica. La ausencia de significancia estadística para `GRUPOGESTAGENOS` no necesariamente indica que no haya diferencias clínicas relevantes entre los grupos, sino que el modelo actual y los datos disponibles no proporcionan evidencia estadística de tal efecto. 


## Columna 2 {.tabset}
-----------------------------------------------------------------------

### Variables que guardan un p<0.05 con Blastulación 
...
### Modelo BLASTUL Primera Estimulación

```{r}
lr <- glm((BLASTUL_1ESTIM=="SI")*1.0 ~ EDAD + AFC + FACTOR_FEM_1,
...
### Modelo BLASTUL Segunda Estimulación

```{r}
lr2 <- glm((BLASTUL_2ESTIM=="SI")*1.0 ~  GRUPO,
                      family = binomial(link = "logit"), data = df_imputado)
tidy_model2 <- broom::tidy(lr2)
datatable(
  tidy_model2 %>%
  mutate_if(is.numeric, round, digits = 4),
  extensions = 'Buttons',
...
### Ajuste Modelo Blastulación Primera Estimulación

```{r message=FALSE, warning=FALSE, include=FALSE}
# Aplicar stepAIC para seleccionar modelo con dirección hacia atrás
modelo_seleccionado <- stepAIC(lr, direction = "backward")

# Utilizar broom para estructurar los resultados del modelo final
modelo_tidy <- tidy(modelo_seleccionado)
```

```{r}
# Presentar la tabla de coeficientes del modelo seleccionado
tidy_model_step <- broom::tidy(modelo_seleccionado)
datatable(
  tidy_model_step %>%
  mutate_if(is.numeric, round, digits = 4),
  extensions = 'Buttons',
...
### Ajuste Modelo Blastulación Segunda Estimulación


```{r message=FALSE, warning=FALSE, include=FALSE}
# Aplicar stepAIC para seleccionar modelo con dirección hacia atrás
modelo_seleccionado2 <- stepAIC(lr2, direction = "forward")

# Utilizar broom para estructurar los resultados del modelo final
modelo_tidy2 <- tidy(modelo_seleccionado2)
```


```{r}
# Presentar la tabla de coeficientes del modelo seleccionado
tidy_model_step2 <- broom::tidy(modelo_seleccionado2)
datatable(
  tidy_model_step2 %>%
  mutate_if(is.numeric, round, digits = 4),
  extensions = 'Buttons',
...
# Código Usado {data-navmenu="Análisis Multivariante"}

## Columna 1 {.tabset}
-----------------------------------------------------------------------
### Filtrado variables con significancia
```{r echo=TRUE, eval = FALSE}
# =======================================
#             Filtrado DPLYR
# =======================================
...
### Blastulación en Primera Estimulación
```{r echo=TRUE, eval = FALSE}
...
# Se utiliza broom para poder mostrar los resultados de una forma más agradable
# Si se necesitan otras medidas (como varianza de los residuos) se añaden 
# manualmente después de consultar summary(lr)
tidy_model <- broom::tidy(lr)
...
#             Ajuste: Stepwise 
#========================================
modelo_seleccionado1 <- stepAIC(lr1, direction = "backward")
