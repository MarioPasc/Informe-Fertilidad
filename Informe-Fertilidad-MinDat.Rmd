...
minimos <- sapply(df_imputado[numericas], min)
q1 <- sapply(df_imputado[numericas], function(x) quantile(x, 0.25))
medianas <- sapply(df_imputado[numericas], median)
medias <- sapply(df_imputado[numericas], mean)
sd <- sapply(df_imputado[numericas], sd)
q3 <- sapply(df_imputado[numericas], function(x) quantile(x, 0.75))
maximos <- sapply(df_imputado[numericas], max)
iqr_values <- q3 - q1

# Calcular outliers
outliers_count <- sapply(seq_along(numericas), function(i) {
  lower_bound <- q1[i] - 1.5 * iqr_values[i]
  upper_bound <- q3[i] + 1.5 * iqr_values[i]
  sum(df_imputado[[numericas[i]]] < lower_bound | df_imputado[[numericas[i]]] > upper_bound)
})
...
renderPlotly({
  ...
  # Generar gráfico
  p <- plot_ly(data = df_imputado, x = ~eval(as.name(input$varX)), y = ~eval(as.name(input$varY))) %>%
    add_markers() %>%
    layout(title = paste("Dispersion entre", input$varX, "y", input$varY),
